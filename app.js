  ...
  function init() {
    ...
    const QUESTIONS = [
      ...
      },
      {
        ...
        answerIndex: 2,
        explanation: 'A straight stream has greater reach and penetration for attacking the seat of the fire.',
        difficulty: 1
      },
      {
        id: 'FFI-HOSE-002',
      ...
      try {
        ...
        if (!res.ok) {
          ...
          if (leaderboardNoteEl) {
            leaderboardNoteEl.textContent = `Sync error (${res.status}). Check console for details.`;
          }
        }
      } catch (e) {
        console.warn('Failed to sync score to Supabase', e);
        if (leaderboardNoteEl) {
          leaderboardNoteEl.textContent = 'Sync error (network). Check console.';
        }
      }
    }
