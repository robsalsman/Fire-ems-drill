// Endurance-enabled Fire/EMS drill app (DOM-ready, safe localStorage)
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  // Guard: if start button is missing, bail (should never happen)
  if (!startBtn) {
  ...
  function handleAnswer(selectedIndex) {
    ...
    if (selectedIndex === q.answerIndex) {
      ...
      if (!state.examMode) {
        feedbackEl.textContent =
          `Time up. Correct answer: ${q.choices[q.answerIndex]}. ${q.explanation}`;
      }
    } else {
      if (!state.examMode) {
